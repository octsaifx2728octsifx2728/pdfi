...
var paySlider_c={
	...
	render:function(){
		...
		this.mainViewerWrap=this.$(document.createElement("div"));
                ...
                this.maintcontainer=this.$(document.createElement("div"));
                
                $(this.maintcontainer).addClass("maintcontainer");
		...
		this.append(this.mainViewer);
                this.append(this.maintcontainer);
                
		this.maintcontainer.append(this.tcontainer);
		...
		var mvw=this.mainViewerWrap.width();
                ...
                this.mainViewerWrap.append('<span data-u="arrowleft" class="jssora02l" style="top: 600px;right: 1105px;z-index:1; width: 55px; height: 55px;" data-autocenter="2"></span>');
                this.mainViewerWrap.append('<span data-u="arrowright" class="jssora02r" style="top: 600px;right: 430px;z-index:1; width: 55px; height: 55px;" data-autocenter="2"></span>');
                
                
                
                
                
                var fg = this.thumbs.length-1;
                $('.jssora02r').click(function(){
                    ...
                    if(fg > item_auxx){
                        $('.image').each(function(x){
                        
                        $(this).css("border","");
                    });
                    
                        paySlider_c.DDBB.objects[slider_aux].change(++item_auxx);
                        
                        $('.image.image_'+(item_auxx+1)).css("border","solid 3px #666");
                        /*
                        var leftPos = $('.maintcontainer').scrollLeft(); 
                        
                        var posx = $('.image.image_'+(item_auxx+1)).offset().left;
                        
                        console.log(leftPos);
                        console.log(posx);
                        
                        $('.maintcontainer').animate({
                            scrollLeft: leftPos + 40
                        }, 800);
                        */
                    }
		...
		for(var i=0,leng=this.thumbs.length;i<leng;i++){
			...
			//cont.mouseover(paySlider_c.changeHandler);
                        var conteni = this.mainViewer;
                        var posisfx = this.posiciones;
                        $(cont).click(function(){
                        ...
                        $('.image.image_'+s.toString()).css("border","solid 3px #666");
                        
                            conteni.animate({scrollLeft:posisfx[item_auxx]},0);
			...
			this.rendedFulls.push(cont2);
                ...
                $('.image.image_1').css("border","solid 3px #666");
                
                
                $(".jssora02l").css('left',$('.mainViewer').offset().left);
                
                
                
            $(".jssora02r").css('left', $('.imagesWrap').height()+$('.imagesWrap').offset().left+86+'px');
                
                $('.maintcontainer').css('width', $('.mainViewer').width()+'px');
